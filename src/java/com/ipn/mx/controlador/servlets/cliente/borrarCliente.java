...
import com.ipn.mx.modelo.pojos.ClienteKey;
...
public class borrarCliente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String estado = "Error al borrar el cliente";
        int id = Integer.valueOf(request.getParameter("txtId"));
        try {
            HttpSession session = request.getSession();
            Connection conexion = (Connection) session.getAttribute("conexion");
            ClienteKey cid = new ClienteKey();
            cid.setIdcliente(id);
            new ClienteDAOImpl().delete(cid, conexion);
            estado = "Cliente borrado con exito";
        } catch (SQLException e) {
            System.err.println("Algo salio mal al borrar un cliente " + e);
        }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("Cliente borrado con  id= " + id);
