...
import com.ipn.mx.modelo.pojos.CompraKey;
...
@WebServlet(name = "borrarCompra", urlPatterns = {"/borrarCompra"})
public class borrarCompra extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String estado = "Error al borrar la compra";
        int id = Integer.valueOf(request.getParameter("txtId"));
        try {
            HttpSession session = request.getSession();
            Connection conexion = (Connection) session.getAttribute("conexion");
            CompraKey ck = new CompraKey();
            ck.setIdcompra(id);
            new CompraDAOImpl().delete(ck, conexion);
            estado = "Compra borrada con exito";
        } catch (SQLException e) {
            System.err.println("Algo salio mal al borrar la compra " + e);
        }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("Compra borrada con id= " + id);
